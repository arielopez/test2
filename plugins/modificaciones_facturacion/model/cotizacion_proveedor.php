 ...
 */
class cotizacion_proveedor extends fs_model
{
   /**
    * Clave primaria.
    * @var type 
   ...
   {
      parent::__construct('cotizacion_prov', 'plugins/facturacion_base/');
      if($a)
      {
         $this->idcotizacion = $this->intval($a['idcotizacion']);
         
         $this->idorden_compra = $this->intval($a['idorden_compra']);
         if($this->idorden_compra == 0)
         {
            $this->idorden_compra = NULL;
         }
         
         $this->ptefactura = $this->str2bool($a['ptefactura']);
         if($this->idorden_compra)
         {
...
//         return 'index.php?page=compras_albaranes';
   ...
   {
      if( is_null($this->idorden_compra) )
      {
         return '#';
      }
      else
   ...
   public function get_lineas()
   {
      ...
      return $linea->all_from_cotizacion($this->idcotizacion);
   }
   
   public function get($id)
   {
      $cotizacion = $this->db->select("SELECT * FROM ".$this->table_name." WHERE idcotizacion = ".$this->var2str($id).";");
      if($cotizacion)
      {
         return new cotizacion_proveedor($cotizacion[0]);
      }
      else
         return FALSE;
   }
   ...
   public function exists()
   {
      if( is_null($this->idcotizacion) )
      {
         ...
         return $this->db->select("SELECT * FROM ".$this->table_name." WHERE idcotizacion = ".$this->var2str($this->idcotizacion).";");
   }
   
   public function new_codigo()
   {
      $sec = new secuencia();
      $sec = $sec->get_by_params2($this->codejercicio, $this->codserie, 'ncotizacionprov');
      if($sec)
      {
      ...
      {
         $this->codigo = strtoupper(substr("cotizacion", 0, 3)).$this->codejercicio.$this->codserie.$this->numero.'C';
      }
   }
   ...
   public function full_test($duplicados = TRUE)
   {
      ...
      {
         $this->new_error_msg("Valor totaliva de Cotizacion incorrecto. Valor correcto: ".$iva);
         $status = FALSE;
      }
      else if( !$this->floatcmp($this->totalirpf, $irpf, FS_NF0, TRUE) )
      {
         $this->new_error_msg("Valor totalirpf de Cotizacion incorrecto. Valor correcto: ".$irpf);
         $status = FALSE;
      }
      else if( !$this->floatcmp($this->totalrecargo, $recargo, FS_NF0, TRUE) )
      {
         $this->new_error_msg("Valor totalrecargo de Cotizacion incorrecto. Valor correcto: ".$recargo);
         $status = FALSE;
      }
      else if( !$this->floatcmp($this->total, $total, FS_NF0, TRUE) )
      {
         $this->new_error_msg("Valor total de Cotizacion incorrecto. Valor correcto: ".$total);
         $status = FALSE;
      }
      else if( !$this->floatcmp($this->totaleuros, $this->total * $this->tasaconv, FS_NF0, TRUE) )
      {
         $this->new_error_msg("Valor totaleuros de Cotizacion incorrecto.
      ...
      if($this->total != 0)
      {
         ...
         $linea_orden = new linea_orden_compra_proveedor();
         $ordenes = $linea_orden->orden_compra_from_cotizacion( $this->idcotizacion );
         if($ordenes)
         {
            if( count($ordenes) > 1 )
            {
               $msg = "Esta Cotizacion esta asociado a las siguientes ordenes (y no debería):";
               foreach($ordenes as $f)
                  $msg .= " <a href='".$f->url()."'>".$f->codigo."</a>";
               $this->new_error_msg($msg);
               $status = FALSE;
            }
            else if($ordenes[0]->idpedido != $this->idorden_compra)
            {
               $this->new_error_msg("Esta Cotizacion esta asociada a una <a href='".$this->factura_url().
                       "'>factura</a> incorrecta. La correcta es <a href='".$ordenes[0]->url()."'>esta</a>.");
               $status = FALSE;
            }
         }
      ...
      if($status AND $duplicados)
      {
         ...
         $data = $this->db->select("SELECT * FROM ".$this->table_name." WHERE fecha = ".$this->var2str($this->fecha)."
            ...
            AND observaciones = ".$this->var2str($this->observaciones)." AND idcotizacion != ".$this->var2str($this->idcotizacion).";");
         if($data)
         {
            foreach($data as $alb)
            {
               /// comprobamos las líneas
               $aux = $this->db->select("SELECT referencia FROM lineascotizacion_prov WHERE
                  idcotizacion = ".$this->var2str($this->idcotizacion)."
                  AND referencia NOT IN (SELECT referencia FROM lineascotizacion_prov
                  WHERE idcotizacion = ".$this->var2str($alb['idcotizacion']).");");
               if( !$aux )
               {
                  $this->new_error_msg("Esta Cotizacion es un posible duplicado de
                     <a href='index.php?page=compras_cotizacion&id=".$alb['idcotizacion']."'>este otro</a>.
   ...
   public function save()
   {
      if( $this->test() )
      {
         if( $this->exists() )
         {
            $sql = "UPDATE ".$this->table_name." SET idorden_compra = ".$this->var2str($this->idorden_compra)
                    ...
                    ."  WHERE idcotizacion = ".$this->var2str($this->idcotizacion).";";
         ...
         else
         {
            ...
            if( $this->db->exec($sql) )
            {
               $this->idcotizacion = $this->db->lastval();
               return TRUE;
            }
            else
               return FALSE;
   ...
   {
      if( $this->db->exec("DELETE FROM ".$this->table_name." WHERE idcotizacion = ".$this->var2str($this->idcotizacion).";") )
      {
         if($this->idorden_compra)
         {
            /**
            ...
            $factura0 = $factura->get($this->idorden_compra);
            if($factura0)
            {
               $factura0->delete();
            }
   ...
   public function all($offset=0, $order='fecha DESC, codigo DESC')
   {
      ...
      {
         foreach($data as $a)
            $albalist[] = new cotizacion_proveedor($a);
      }
   ...
   public function all_from_proveedor($codproveedor, $offset=0)
   {
      ...
      {
         foreach($data as $a)
            $alblist[] = new cotizacion_proveedor($a);
      }
   ...
   {
      /*
       * Marcamos como ptefactura = TRUE todos los cotizaciones de ejercicios
