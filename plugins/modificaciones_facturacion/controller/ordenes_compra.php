...
class ordenes_compra extends fs_controller
{
   ...
   {
      parent::__construct(__CLASS__, 'Ordenes de compra', 'compras');
//      parent::__construct(__CLASS__, ucfirst(FS_PEDIDOS).' de compra', 'compras');
   ...
   {
      $data = $this->db->select("SELECT COUNT(idpedido) as total FROM orden_compra_prov WHERE idalbaran IS NULL;");
      if($data)
      {
