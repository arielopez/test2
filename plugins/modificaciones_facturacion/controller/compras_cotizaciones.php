...
class compras_cotizaciones extends fs_controller
{
   public $agente;
   public $articulo;
   ...
   {
      parent::__construct(__CLASS__, 'Cotizaciones de compra', 'compras', FALSE, TRUE, TRUE);
   }
   ...
   protected function private_core()
   {
      ...
      {
         $this->template = 'extension/compras_cotizaciones_articulo';
      ...
      else
      {
         ...
         {
            $this->resultados = $cotizacion->all_ptefactura($this->offset, $this->order.$order2);
         ...
         else if($this->mostrar == 'buscar')
         {
            ...
            $this->resultados = $cotizacion->all($this->offset, $this->order.$order2);
      }
   }
   ...
   public function buscar_lineas()
   {
      ...
      $this->template = 'ajax/compras_lineas_cotizaciones';
   ...
   private function delete_cotizacion()
   {
      ...
      if($alb1)
      {
         ...
         if( $alb1->delete() )
         {
            $this->new_message("Cotizacion ".$alb1->codigo." borrado correctamente.");
         }
         else
            $this->new_error_msg("¡Imposible borrar la Cotizacion !");
      }
      else
         $this->new_error_msg("¡Cotizacion no encontrada!");
   }
   ...
   private function share_extension()
   {
      ...
      $extensiones = array(
          array(
              'name' => 'cotizaciones_proveedor',
              'page_from' => __CLASS__,
              'page_to' => 'compras_proveedor',
              'type' => 'button',
              'text' => '<span class="glyphicon glyphicon-list" aria-hidden="true"></span> &nbsp; '.ucfirst("Cotizaciones"),
              'params' => ''
          ),
          array(
              'name' => 'cotizaciones_agente',
              'page_from' => __CLASS__,
              'page_to' => 'admin_agente',
              'type' => 'button',
              'text' => '<span class="glyphicon glyphicon-list" aria-hidden="true"></span> &nbsp; '.ucfirst("Cotizaciones").' de proveedor',
              'params' => ''
          ),
          array(
              'name' => 'cotizaciones_articulo',
              'page_from' => __CLASS__,
   ...
   {
      $data = $this->db->select("SELECT COUNT(idcotizacion) as total FROM cotizacion_prov WHERE ptefactura;");
      if($data)
      {
   ...
   {
      $data = $this->db->select("SELECT COUNT(idcotizacion) as total FROM cotizacion_prov;");
      if($data)
      {
         return intval($data[0]['total']);
      }
   ...
   private function buscar($order2)
   {
      ...
      $data = $this->db->select("SELECT COUNT(idcotizacion) as total".$sql);
      if($data)
      {
         ...
         if($data2)
         {
            foreach($data2 as $d)
            {
               $this->resultados[] = new cotizacion_proveedor($d);
            }
         }
